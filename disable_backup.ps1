...
$speclist = (.\plink.exe -ssh -l $UNIXUSER -i .\$PUBKEY $script:cellname "ls -d /etc/opt/omni/server/schedules/* |grep -i $script:specname ; ls -d /etc/opt/omni/server/barschedules/oracle8/* |grep -i $script:specname ;ls -d /etc/opt/omni/server/barschedules/mssql/* |grep -i $script:specname") 
...
gc .\specfile.txt |ForEach-Object{ 'if [ -r ' + $_ + ' ] ; then ' + 'cp' + " " + $_ + " " + '/home/$UNIXUSER/sched/' +($_.Split("/")[6,7] -join "_") + "_" + $date_now + ' ; else echo " No Read Permission ' + ($_.Split("/")[6,7] -join " ")+ '" ; fi' } |Out-File .\cmd.txt                                                                   #copy schedule to homedir
#gc .\specfile.txt |ForEach-Object{ (gc .\perl.txt)[0] + "/home/$UNIXUSER/sched/" + ($_.Split("/")[4,5] -join "_") + "_" +$script:date_now + " | tee " + $_ } |Out-File -Append .\cmd.txt                                          #remove existing disable
#gc .\specfile.txt |ForEach-Object{ (gc .\perl.txt)[1] + "/home/$UNIXUSER/sched/" + ($_.Split("/")[4,5] -join "_") + "_" +$script:date_now + " | tee " + $_ } |Out-File -Append .\cmd.txt                                          #remove existing starting
gc .\specfile.txt |ForEach-Object{ 'if [ -r ' + $_ + ' ] ; then ' + 'cat ' + $_ + (gc .\perl.txt)[7] + " | tee /home/$UNIXUSER/schedtmp/" + ($_.Split("/")[6,7] -join "_") + "_" +$date_now + ' > /dev/null' + ' ; else echo "Schedule not modified" ; fi ' } | Out-File -Append .\cmd.txt                  #grep -v disable and starting and output to temp file
gc .\specfile.txt |ForEach-Object{ 'if [ -r ' + $_ + ' ] ; then ' + (gc .\perl.txt)[2] + "/home/$UNIXUSER/schedtmp/" + ($_.Split("/")[6,7] -join "_") + "_" +$date_now + " | /opt/pb/bin/pbrun tee " + $_ + ' > /dev/null 2>&1 ; echo "Disable Successful" '  + ' ; else echo "Please check file permission" ; fi ' } |Out-File -Append .\cmd.txt                                          #add disable to first line
#gc .\specfile.txt |ForEach-Object{ 'if cmp -s ' + $_ + '/home/$UNIXUSER/sched/' +($_.Split("/")[6,7] -join "_") + "_" + $date_now + ' ; then echo "Modify Success" ; else echo "Modify Failed" ; fi' } | Out-File -Append .\cmd.txt
...
(.\plink.exe -ssh -l $UNIXUSER -i .\$PUBKEY $script:cellname -m .\cmd.txt)
}
...
#gc .\specfile.txt |ForEach-Object{ (gc .\perl.txt)[0] + "/home/aqmalbkp/sched/" + ($_.Split("/")[4,5] -join "_") + "_" + $script:date_now + " | tee " + $_  } |Out-File -Append .\cmd.txt                                          #remove existing disable
#gc .\specfile.txt |ForEach-Object{ (gc .\perl.txt)[1] + "/home/aqmalbkp/sched/" + ($_.Split("/")[4,5] -join "_") + "_" + $script:date_now + " | tee " + $_  } |Out-File -Append .\cmd.txt                                          #remove existing starting
gc .\specfile.txt |ForEach-Object{ 'if [ -r ' + $_ + ' ] ; then ' + 'cat ' + '/home/$UNIXUSER/sched/' + ($_.Split("/")[6,7] -join "_") + "_" + $:date_now + (gc .\perl.txt)[7] + " | /opt/pb/bin/pbrun tee " + $_ + ' > /dev/null 2>&1 ; echo "Enable Successful" '  + ' ; else echo "Schedule not modified" ; fi ' } | Out-File -Append .\cmd.txt
Add-Content -value "/opt/omni/bin/omnirpt -rep dl_sched -tab |grep $script:specname" -path .\cmd.txt
[System.Environment]::CurrentDirectory = (Get-Location).Path
$MyPath = ".\cmd.txt"
$MyFile = Get-Content $MyPath
